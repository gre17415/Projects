...
void PeerConnect::PerformHandshake() {
    ...
    std::cout << "Successful handshake, peer id = " << peerId_ << std::endl;
}
...
void PeerConnect::RequestPiece() {
    ...
    if (!pendingBlock_) {
        ...
        if (block) {
            ...
            Message msg = Message::Init(MessageId::Request, requestPayload);
            socket_.SendData(msg.ToString());
            std::cout << "Requested piece " << pieceInProgress_->GetIndex()
    ...
    while (!terminated_) {
        ...
        case MessageId::Piece: {
            ...
            if (pieceInProgress_ && pieceInProgress_->GetIndex() == pieceIndex) {
                ...
                std::cout << "Piece block received, offset=" << offset
                          << ", size=" << blockData.size() << std::endl;
        ...
        case MessageId::Unchoke:
            ...
            break;
        default:
            std::cout << "Received message id=" << static_cast<int>(msg.id) << std::endl;
            break;
        }
