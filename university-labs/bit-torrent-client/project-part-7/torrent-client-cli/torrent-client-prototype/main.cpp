...
void CheckDownloadedPiecesIntegrity(const fs::path& outputFilename, const TorrentFile& tf, PieceStorage& pieces) {
    pieces.CloseOutputFile();

    if (fs::file_size(outputFilename) != tf.length) {
...
void DeleteDownloadedFile(const fs::path& outputFilename) {
    fs::remove(outputFilename);
}

fs::path PrepareDownloadDirectory(const std::string& outputDirectory) {
    fs::create_directories(outputDirectory);
    return outputDirectory;
}

bool RunDownloadMultithread(PieceStorage& pieces, const TorrentFile& torrentFile,
    ...
    while (pieces.PiecesSavedToDiscCount() < PiecesToDownload) {
        if (pieces.PiecesInProgressCount() == 0) {
            {
                std::lock_guard<std::mutex> coutLock(coutMutex);
                std::cout << "Want to download more pieces but all peer connections are not working. "
                          << "Let's request new peers" << std::endl;
            }
            for (auto& peerConnectPtr : peerConnections) {
...
void TestTorrentFile(const fs::path& file, const fs::path& outputDir, int percent) {
    ...
    const fs::path outputDirectory = PrepareDownloadDirectory(outputDir.string());
    PieceStorage pieces(torrentFile, outputDirectory);
    PiecesToDownload = (torrentFile.pieceHashes.size() * percent + 99) / 100; // округление вверх
...
int main(int argc, char** argv) {
    if (argc < 6) {
        std::cerr << "Usage: " << argv[0] << " -d <output_dir> -p <percent> <torrent_file>" << std::endl;
        return 1;
    }
    ...
    if (torrentFile.empty()) {
        std::cerr << "Torrent file not specified" << std::endl;
        return 1;
    }

    try {
        TestTorrentFile(torrentFile, outputDir, percent);
    } catch (const std::exception& e) {
